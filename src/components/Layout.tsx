      ...
      <header className="relative z-10 border-b-4 border-border-subtle bg-bg-secondary">
        <div className="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-accent flex items-center justify-center text-2xl font-serif text-bg-primary shadow-lg" style={{ clipPath: 'polygon(50% 0%, 100% 50%, 50% 100%, 0% 50%)' }}>
              G
            </div>
            <div>
          ...
          <nav className="hidden md:flex items-center gap-1">
            {STEPS.map((step, index) => {
              ...
              return (
                ...
                >
                  {isActive && (
                    <div className="absolute inset-0 border border-accent opacity-50" style={{ clipPath: 'polygon(8px 0, 100% 0, 100% calc(100% - 8px), calc(100% - 8px) 100%, 0 100%, 0 8px)' }} />
                  )}
                  <span className={isActive ? "brutalist-number text-lg" : "font-sans text-lg"}>{isCompleted ? '✓' : index + 1}</span>
                  <span className="uppercase font-light tracking-widest text-[10px]">{step.label}</span>
                </button>
              );
            })}
          </nav>
